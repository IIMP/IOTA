 ...
 * 
 */
public class IotaAPICorecompass {
    private static final Logger log = LoggerFactory.getLogger(IotaAPICorecompass.class);
    ...
    protected IotaAPICorecompass(ApiOptions options) {
     ...
     *                  Can be <tt>null</tt>.
     * @return {@link GetTransactionsToApproveResponsecompass}
     * @throws ArgumentException The request was considered wrong in any way by the node
     * @throws ArgumentException The depth is invalid
     */
    @Document
    public GetTransactionsToApproveResponsecompass getTransactionsToApprovecompass(Integer depth, String reference) throws ArgumentException {
        if (depth < 0) {
            throw new ArgumentException(INVALID_APPROVE_DEPTH_ERROR);
        }
        
        return getNodeFor(IotaAPICommand.GET_TRANSACTIONS_TO_APPROVEcompass).getTransactionsToApprovecompass(IotaGetTransactionsToApproveRequestcompass.createIotaGetTransactionsToApproveRequestcompass(depth, reference));
    }
     ...
     */
    @Document
    public GetTransactionsToApproveResponsecompass getTransactionsToApprove(Integer depth) throws ArgumentException {
        return getTransactionsToApprovecompass(depth, null);
    }
