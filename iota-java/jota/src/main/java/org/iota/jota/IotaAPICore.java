...
public class IotaAPICore {
    ...
    @Document
    public GetStorageResponse getStorage(String addr, Integer key) throws ArgumentException{
        return getNodeFor(IotaAPICommand.GET_STORAGE).getStorage(IotaGetStorageRequest.createIotaGetStorageRequest(addr,key));
    }
    /**
     * Tip selection which returns <tt>trunkTransaction</tt> and <tt>branchTransaction</tt>.
     * The input value <tt>depth</tt> determines how many milestones to go back for finding the transactions to approve.
    ...
    @Document
    public GetTransactionsToApproveResponsecompass getTransactionsToApprovecompass(Integer depth) throws ArgumentException {
