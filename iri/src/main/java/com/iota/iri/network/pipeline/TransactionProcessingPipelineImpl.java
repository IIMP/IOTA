...
public class TransactionProcessingPipelineImpl implements TransactionProcessingPipeline {
    ...
    private final ExecutorService stagesThreadPool = Executors.newFixedThreadPool(NUMBER_OF_THREADS);
    ...
    private final BatchedHasher batchedHasher;
    private HashingStage hashingStage;

    private final BlockingQueue<ProcessingContext> preProcessStageQueue = new ArrayBlockingQueue<>(100);
    private final BlockingQueue<ProcessingContext> validationStageQueue = new ArrayBlockingQueue<>(100);
    private final BlockingQueue<ProcessingContext> receivedStageQueue = new ArrayBlockingQueue<>(100);
    private final BlockingQueue<ProcessingContext> broadcastStageQueue = new ArrayBlockingQueue<>(100);
    private final BlockingQueue<ProcessingContext> replyStageQueue = new ArrayBlockingQueue<>(100);
     ...
     *                               reply stage
     */
    public TransactionProcessingPipelineImpl(final NeighborRouter neighborRouter, final NodeConfig config,
            final TransactionValidator txValidator, final Tangle tangle, final SnapshotProvider snapshotProvider,
            final TipsViewModel tipsViewModel, final LatestMilestoneTracker latestMilestoneTracker,
            final TransactionRequester transactionRequester) {
        final FIFOCache<Long, Hash> recentlySeenBytesCache = new FIFOCache<>(config.getCacheSizeBytes());
     ...
     */
    private void addStage(final String name, final BlockingQueue<ProcessingContext> queue,
            final com.iota.iri.network.pipeline.Stage stage) {
        stagesThreadPool.submit(new Thread(() -> {
            try {
                while (!Thread.currentThread().isInterrupted()) {
                    final ProcessingContext ctx = stage.process(queue.take());
                    switch (ctx.getNextStage()) {
                        ...
                        case RECEIVED:
                            ...
                            break;
                        case MULTIPLE:
                            final MultiStagePayload payload = (MultiStagePayload) ctx.getPayload();
    ...
    @Override
    public void process(final Neighbor neighbor, final ByteBuffer data) {
        try {
    ...
    @Override
    public void process(final byte[] txTrits) {
        final byte[] txBytes = new byte[Transaction.SIZE];
        Converter.bytes(txTrits, txBytes);
        final long txDigest = TransactionCacheDigester.getDigest(txBytes);
        final HashingPayload payload = new HashingPayload(null, txTrits, txDigest, null);
     ...
     */
    private void hashAndValidate(final ProcessingContext ctx) {
        // the hashing already runs in its own thread,
        // the callback will submit the data to the validation stage
        final HashingPayload hashingStagePayload = (HashingPayload) ctx.getPayload();
    ...
    @Override
    public void setPreProcessStage(final PreProcessStage preProcessStage) {
        this.preProcessStage = preProcessStage;
    }

    @Override
    public void setReceivedStage(final ReceivedStage receivedStage) {
        this.receivedStage = receivedStage;
    }

    @Override
    public void setValidationStage(final ValidationStage validationStage) {
        this.validationStage = validationStage;
    }

    @Override
    public void setReplyStage(final ReplyStage replyStage) {
        this.replyStage = replyStage;
    }

    @Override
    public void setBroadcastStage(final BroadcastStage broadcastStage) {
        this.broadcastStage = broadcastStage;
    }

    @Override
    public void setHashingStage(final HashingStage hashingStage) {
