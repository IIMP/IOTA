...
public class Tangle {
    ...
    public void init() throws Exception {
        ...
        byte[] root = null;
        try{
            root = file2byte("dbRoot.txt");
        }catch(Exception e){
            log.info("DBRoot load failed");
        }
        stateDS = new RocksDbDataSource();
        stateDS.setName("EVM");
        stateDS.init(DbSettings.DEFAULT);
        pruningStateDS = new JournalSource<>(stateDS);
        final RepositoryRoot repo = new RepositoryRoot(pruningStateDS,root);
    ...
    public void shutdown() throws Exception {
        ...
        byte[] root = this.repository.getRoot();
        byte2file("dbRoot.txt", root);
