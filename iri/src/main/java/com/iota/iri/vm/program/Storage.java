...
import com.iota.iri.vm.ByteArrayWrapper;
...
import com.iota.iri.vm.program.listener.ProgramListener;
...
public class Storage implements Repository, ProgramListenerAware {
    ...
    // @Override boolean aliasAccount(byte[] a, byte[] b, byte[] c){
    //     Logger.logMsg(Logger.DEBUG, "called aliasAccount at class "+ this.getClass().getName());
    //     return false;
    // }
    ...
    @Override
    public AccountMap getAccountMap(byte[] addr){
        return repository.getAccountMap(addr);
    }

    @Override 
    public boolean aliasAccount(byte[] a, byte[] b, byte[] c){
        return repository.aliasAccount(a,b,c);
    }
