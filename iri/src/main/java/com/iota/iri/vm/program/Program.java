 ...
 */
package com.iota.iri.vm.program;
...
import com.iota.iri.vm.vm.PrecompiledContracts.PrecompiledContract;
...
import com.iota.iri.vm.program.listener.CompositeProgramListener;
import com.iota.iri.vm.program.listener.ProgramListenerAware;
import com.iota.iri.vm.program.listener.ProgramStorageChangeListener;
// import com.iota.iri.vm.trace.ProgramTraceListener;
// import com.iota.iri.vm.trace.ProgramTrace;
...
public class Program {
    ...
    public Program(byte[] ops, ProgramInvoke programInvoke) {
        this(ops, programInvoke, (EVMTransaction) null);
    }
    ...
    public Program(byte[] ops, ProgramInvoke programInvoke, EVMTransaction transaction) {
        this(ops, programInvoke, transaction, SystemProperties.getDefault(), VMHook.EMPTY);
    }

    public Program(byte[] ops, ProgramInvoke programInvoke, EVMTransaction transaction, SystemProperties config, VMHook vmHook) {
        this(null, ops, programInvoke, transaction, config, vmHook);
    }

    public Program(byte[] codeHash, byte[] ops, ProgramInvoke programInvoke, EVMTransaction transaction, SystemProperties config, VMHook vmHook) {
        this.config = config;
        ...
        this.result.getStorageAKVHash().add(DatatypeConverter.printHexBinary(transaction.getData()));
        // System.out.println("Proggram===========>added result storageAKVHash:"+;
    }
    ...
    private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress) {
        ...
        ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
                this, DataWord.of(newAddress), getOwnerAddress(), value,
                newBalance, null, track, originalRepo, false, byTestingSuite());
        ...
        if (!result.isRevert() && result.getException() == null) {
            ...
            long storageCost = getLength(code) * 0;
            long afterSpend = 0;
            if (afterSpend < 0) {
    ...
    public void callToAddress(MessageCall msg) {
        ...
        if (isNotEmpty(programCode)) {
            ProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
                    ...
                    contextBalance, data, track, this.invoke.getOrigRepository(),
    ...
    public static class Exception {
        ...
        public static OutOfGasException notEnoughSpendingGas(String cause, long gasValue, Program program) {
            return new OutOfGasException("Not enough gas for '%s' cause spending: invokeGas[%d], gas[%d], usedGas[%d];",
                    cause, 0, gasValue, program.getResult().getGasUsed());
        }
