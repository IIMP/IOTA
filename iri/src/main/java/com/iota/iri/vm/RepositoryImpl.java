 ...
 */
public class RepositoryImpl implements Repository, com.iota.iri.vm.facade.Repository {

    protected RepositoryImpl parent;
    protected Source<byte[], AccountMap> accountMapCache;
    ...
    public RepositoryImpl(Source<byte[], AccountMap> accountMapCache, Source<byte[], AccountState> accountStateCache, Source<byte[], byte[]> codeCache,
                          MultiCache<? extends CachedSource<DataWord, DataWord>> storageCache) {
        init(accountMapCache, accountStateCache, codeCache, storageCache);
    }

    protected void init(Source<byte[], AccountMap> accountMapCache, Source<byte[], AccountState> accountStateCache, Source<byte[], byte[]> codeCache,
                        MultiCache<? extends CachedSource<DataWord, DataWord>> storageCache) {
    ...
    public synchronized boolean aliasAccount(byte[] IOTAAddr, byte[] EVMAddr, byte[] newIOTAAddr){
        try{
        if(Converter.bytesToTrytes(newIOTAAddr).equals("9999999999999999999999999999999999999999999999999999999999999999999999999999999999") || newIOTAAddr == null){
        accountMapCache.delete(IOTAAddr);
        AccountMap mp = new AccountMap(EVMAddr);
        accountMapCache.put(IOTAAddr, mp);
        return true;
        }
        else{
            AccountMap mp = accountMapCache.get(IOTAAddr);
            accountMapCache.delete(IOTAAddr);
            accountMapCache.put(newIOTAAddr,mp);
            return true;
    ...
    public synchronized AccountMap getAccountMap(byte[] addr){
        return accountMapCache.get(addr);
    }
    ...
    public synchronized DataWord getStorageValue(byte[] addr, DataWord key) {
        AccountState accountState = getAccountState(addr);
        // System.out.println("=============AccountState is null?"+ (accountState==null)+"================");
    ...
    public synchronized RepositoryImpl startTracking() {
        Source<byte[], AccountMap> trackAccountMapCache = new WriteCache.BytesKey<>(accountMapCache, WriteCache.CacheType.SIMPLE);
        ...
        RepositoryImpl ret = new RepositoryImpl(trackAccountMapCache ,trackAccountStateCache, trackCodeCache, trackStorageCache);
        ret.parent = this;
        return ret;
    }
