 ...
 */
package com.iota.iri.vm.datasource;

import java.math.BigInteger;

import com.iota.iri.vm.AccountMap;
import com.iota.iri.vm.AccountState;
import com.iota.iri.vm.core.BlockHeader;
...
public class Serializers {
    ...
    public final static Serializer<AccountMap, byte[]> AccountMapSerializer = new Serializer<AccountMap, byte[]>() {
        @Override
        public byte[] serialize(AccountMap object) {
            return object.getEncoded();
        }

        @Override
        public AccountMap deserialize(byte[] stream) {
            return stream == null || stream.length == 0 ? null : new AccountMap(stream,BigInteger.ZERO,true);
        }
    };
