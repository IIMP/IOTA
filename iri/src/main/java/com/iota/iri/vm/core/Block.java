...
public class Block {
    ...
    /* Transactions */
    private List<EVMTransaction> transactionsList = new CopyOnWriteArrayList<>();
    ...
    // public Block(BlockHeader header, List<EVMTransaction> transactionsList, List<BlockHeader> uncleList) {
    ...
    //              List<EVMTransaction> transactionsList, List<BlockHeader> uncleList) {
    ...
    public List<EVMTransaction> getTransactionsList() {
        parseRLP();
    ...
    public String toString() {
        ...
        if (!getTransactionsList().isEmpty()) {
            ...
            for (EVMTransaction tx : getTransactionsList()) {
    ...
    private byte[] parseTxs(RLPList txTransactions, boolean validate) {
        ...
        for (int i = 0; i < txTransactions.size(); i++) {
            ...
            EVMTransaction tx = new EVMTransaction(transactionRaw.getRLPData());
    ...
    private byte[] getTransactionsEncoded() {
        ...
        int i = 0;
        for (EVMTransaction tx : transactionsList) {
    ...
    public static final MemSizeEstimator<Block> MemEstimator = block -> {
        ...
        long txSize = block.transactionsList.stream().mapToLong(EVMTransaction.MemEstimator::estimateSize).sum() + 16;
