...
import static com.iota.iri.vm.datasource.MemSizeEstimator.ByteArrayEstimator;
import static com.iota.iri.vm.util.ByteUtil.EMPTY_BYTE_ARRAY;
import static com.iota.iri.vm.util.ByteUtil.ZERO_BYTE_ARRAY;
...
public class EVMTransaction {
    ...
    private static final Logger logger = LoggerFactory.getLogger(EVMTransaction.class);
    ...
    public EVMTransaction(byte[] rawData) {
        this.rlpEncoded = rawData;
        parsed = false;
    }

    public EVMTransaction(byte[] nonce, byte[] sendAddress, byte[] receiveAddress, byte[] value, byte[] data,
                       Integer chainId) {
        this.nonce = nonce;
        if(Arrays.equals(receiveAddress,DatatypeConverter.parseHexBinary("0000000000000000000000000000000000000000"))){
    ...
    public EVMTransaction(byte[] nonce, byte[] receiveAddress, byte[] value, byte[] data,Integer chainId) {
        this.nonce = nonce;
        this.receiveAddress = receiveAddress;
        if (ByteUtil.isSingleZero(value)) {
            this.value = EMPTY_BYTE_ARRAY;
    ...
    // {
    //     this(nonce, receiveAddress, value, data, null);
    // }
    ...
    public byte[] getEncodedRaw() {
        ...
        if (chainId == null) {
            rlpRaw = RLP.encodeList(nonce, receiveAddress,
                    value, data);
        } else {
            byte[] v, r, s;
    ...
    public synchronized byte[] getEncoded() {
        ...
        this.rlpEncoded = RLP.encodeList(nonce,
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof EVMTransaction)) return false;
        EVMTransaction tx = (EVMTransaction) obj;
    ...
    //  */
    // public static EVMTransaction createDefault(String to, BigInteger amount, BigInteger nonce){
    //     return create(to, amount, nonce);
    // }

    // public static EVMTransaction createDefault(String to, BigInteger amount, BigInteger nonce, Integer chainId){
    ...
    //  */
    // public static EVMTransaction create(String to, BigInteger amount, BigInteger nonce){
    //     return new EVMTransaction(BigIntegers.asUnsignedByteArray(nonce),
    ...
    // public static EVMTransaction create(String to, BigInteger amount, BigInteger nonce, Integer chainId){
    //     return new EVMTransaction(BigIntegers.asUnsignedByteArray(nonce),
    //             // BigIntegers.asUnsignedByteArray(gasPrice),
    //             // BigIntegers.asUnsignedByteArray(gasLimit),
    //             Hex.decode(to),
    ...
    public static final MemSizeEstimator<EVMTransaction> MemEstimator = tx ->
