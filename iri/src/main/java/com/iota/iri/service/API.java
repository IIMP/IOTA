...
public class API {
    ...
    public static int EVMAddrLen = 120;
    ...
    public ProgramInvokeFactory programInvokeFactory = new ProgramInvokeFactoryImpl();
    ...
    public API(IotaConfig configuration, IXI ixi, TransactionRequester transactionRequester,
        ...
        commandRoute.put(ApiCommand.GET_STORAGE,getStorage());
        ...
        commandRoute.put(ApiCommand.GET_TRANSACTIONS_TO_APPROVEcompass, getTransactionsToApprovecompass());
    ...
    private Pair<List<String>, List<String>> getSummaries(List<Hash> tips){
        Repository track = getRepository().startTracking();
        // log.info("Getting summaries {}",getLineInfo());
        List<String> trunkSummary,branchSummary;

        if(tips.get(0).equals(tips.get(1))){
            trunkSummary = getSummary(tips.get(0), track);
            track.rollback();
            if(trunkSummary!= null){
                return new Pair<List<String>, List<String>>(trunkSummary, trunkSummary);
            }
        }else{
            trunkSummary = getSummary(tips.get(0), track);
            branchSummary = getSummary(tips.get(1), track);
            track.rollback();
            if(trunkSummary!= null && branchSummary!=null){
                return new Pair<List<String>, List<String>>(trunkSummary, branchSummary);
            }
        }
    ...
    private List<String> getSummary(Hash hash, Repository track){
        ...
        if(!ledgerService.isTransactionConfirmed(trunk)){
            summaries.addAll(getSummary(trunk.getHash(),track));
        }
        if(!ledgerService.isTransactionConfirmed(branch)){
        ...
        for(int i = 0; i < bundleTransactions.size(); i++){
            ...
            if(tag.startsWith("SMARTCONTRACT")){
                ...
                while(i < bundleTransactions.size()){
                    ...
                    executor.execute();
                    executor.go();
                    // log.info("Smart contract executed");
                    // txTrack.commit();

                    TransactionExecutionSummary summary = executor.finalization();
                    // System.out.println("MilestoneSummary<=============="+summary.getEncoded().hashCode()+"");
            ...
            if(tag.startsWith("ALIAS")){
                ...
                String IOTAAddr =  Converter.trytes(Arrays.copyOfRange(trits,TransactionViewModel.ADDRESS_TRINARY_OFFSET, TransactionViewModel.ADDRESS_TRINARY_OFFSET+TransactionViewModel.ADDRESS_TRINARY_SIZE));
                byte[] toAddr = Converter.trytesToBytes(Converter.trytes(Arrays.copyOfRange(trits,TransactionViewModel.SIGNATURE_MESSAGE_FRAGMENT_TRINARY_OFFSET, TransactionViewModel.SIGNATURE_MESSAGE_FRAGMENT_TRINARY_OFFSET + EVMAddrLen)));
                byte[] newAddr = Converter.trytesToBytes(Converter.trytes(Arrays.copyOfRange(trits,TransactionViewModel.SIGNATURE_MESSAGE_FRAGMENT_TRINARY_OFFSET+ EVMAddrLen, TransactionViewModel.SIGNATURE_MESSAGE_FRAGMENT_TRINARY_OFFSET + EVMAddrLen + TransactionViewModel.ADDRESS_TRINARY_SIZE)));
    ...
    /**
     * Convert a message in trytes string to a evmTransaction
     */
    ...
    @Document(name="getTransactionsToApprovecompass")
    private synchronized AbstractResponse getTransactionsToApproveStatementcompass(int depth, Optional<Hash> reference) {
        if (depth < 0 || depth > configuration.getMaxDepth()) {
            return ErrorResponse.create("Invalid depth input");
        }

        try {
            // log.info("==========Compass API called=================");
            List<Hash> tips = getTransactionToApproveTipscompass(depth, reference);
            // System.out.println("===============Tips selected=============");
            Pair<List<String>, List<String>> summaries = getSummaries(tips);
            // System.out.println("===============Summaries got=============");

            String tips0Summary = "";
            String tips1Summary = "";
            if(!summaries.low.isEmpty()){
                for(String tips0 : summaries.low){
                    tips0Summary += Integer.toString(tips0.hashCode());
                }
            }
            if(!summaries.hi.isEmpty()){
                for(String tips1 : summaries.hi){
            ...
            // log.info("[Hash]before: "+summaryHash+" ============= after: "+Converter.asciiToTrytes(summaryHash));
            
            return GetTransactionsToApproveResponsecompass.create(tips.get(0),tips.get(1), finalSummary);
            //CZTBD
        } catch (Exception e) {
            log.info("Tip selection failed: " + e.getLocalizedMessage());
            e.printStackTrace();
    ...
    List<Hash> getTransactionToApproveTipscompass(int depth, Optional<Hash> reference) throws Exception {
        if (!isNodeSynchronized()) {
            throw new IllegalStateException(INVALID_SUBTANGLE);
        }
        ...
        List<Hash> tips;
        try {
            tipSelection = tipSelExecService.submit(() -> tipsSelector.getTransactionsToApprovecompass(depth, reference));
            tips = tipSelection.get(configuration.getTipSelectionTimeoutSec(), TimeUnit.SECONDS);
        } catch (TimeoutException ex) {
            // interrupt the tip-selection thread so that it aborts
            tipSelection.cancel(true);
    ...
    @Document(name = "getStorage")
    private AbstractResponse getStorageStatement(String addr, Integer key){
        Repository repository = getRepository();
        byte[] scAddr = DatatypeConverter.parseHexBinary(addr);
        ContractDetails contractDetails = repository.getContractDetails(scAddr);
        DataWord val = contractDetails.get(DataWord.of(key));
        byte[] ret = val.getData();
        return GetStorageResponse.create(ret);
    }
    ...
    private String getParameterAsString(Map<String, Object> request, String paramName, int size) throws ValidationException {
        validateParamExists(request, paramName);
        String result = (String) request.get(paramName);
        return result;
    } 
    ...
    private Function<Map<String, Object>, AbstractResponse> getStorage(){
        return request -> {
            final String addr = getParameterAsString(request, "addr", 40);
            final Integer key = getParameterAsInt(request, "key");
            try{
                return getStorageStatement(addr, key);
            }catch(Exception e){
                throw new IllegalStateException(e);
            }
        };
    ...
    private Function<Map<String, Object>, AbstractResponse> getTransactionsToApprovecompass() {
        log.info("=========Compass api invoked===========");
        return request -> {
            Optional<Hash> reference = request.containsKey("reference") ?
                Optional.of(HashFactory.TRANSACTION.create(getParameterAsStringAndValidate(request,"reference", HASH_SIZE)))
                : Optional.empty();
            ...
            return getTransactionsToApproveStatementcompass(depth, reference);
        };
    }
